...
async def log_user_query(
    ...
    response_type: str = "answer",  # ⬅️ ADD THIS
    language_detected: str = "en"   # ⬅️ ADD THIS
) -> Dict:
    ...
    query_record = {
        ...
        "error": error,
        "response_type": response_type,      # ⬅️ ADD THIS
        "language_detected": language_detected,  # ⬅️ ADD THIS
...
async def get_user_queries(
    ...
    response_type: Optional[str] = None  # ⬅️ ADD THIS FOR FILTERING
) -> Tuple[List[Dict], int]:
    ...
    # Optional: Filter by recent days
    if days:
        since = datetime.utcnow() - timedelta(days=days)
        query["created_at"] = {"$gte": since}
    
    # Optional: Filter by response type
    if response_type:
...
async def search_user_queries(
    ...
    # Search in both question and answer fields (case-insensitive)
    query = {
        "user_id": ObjectId(user_id),
        "$or": [
