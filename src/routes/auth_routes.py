...
# N8N Webhook Token (shared between backend and n8n)
# This can be the same as JWT or a separate token
...
async def login(
    ...
    try:
        ...
        # Create n8n webhook token (same as JWT or separate)
        n8n_token = access_token  # Use same token for simplicity
        ...
        return LoginResponse(
            ...
            n8n_token=n8n_token,  # Add this to response
            user=UserResponse(
                _id=user.id,
                username=user.username,
                email=user.email,
...
async def get_n8n_token(
    ...
    # Create a token specifically for n8n
    n8n_token = create_access_token(
        ...
        expires_delta=timedelta(days=30)  # Longer expiry for automation
    )
